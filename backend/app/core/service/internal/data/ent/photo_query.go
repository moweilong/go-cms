...
func (pq *PhotoQuery) IDs(ctx context.Context) (ids []uint32, err error) {
...
func (pq *PhotoQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(photo.Table, photo.Columns, sqlgraph.NewFieldSpec(photo.FieldID, field.TypeUint32))
	_spec.From = pq.sql
...
func (pq *PhotoQuery) ForUpdate(opts ...sql.LockOption) *PhotoQuery {
...
func (pq *PhotoQuery) ForShare(opts ...sql.LockOption) *PhotoQuery {
	if pq.driver.Dialect() == dialect.Postgres {
		pq.Unique(false)
	}
	pq.modifiers = append(pq.modifiers, func(s *sql.Selector) {
